...
from .draw_rect_processor import *


class DrawRect(ModelBasedArcAction[TrainingAttentionTask, AttentionTask]):
    def __init__(self, selection: SelectCondition, feat_index: int,
                 old_color_model: MLModel, new_color_model: MLModel,
                 params: GlobalParams, min_width: int = 2, min_height: int = 2)->None:
    ...
    def _perform(self, shape: Shape, old_color: int, new_color: int)->Optional[Shape]:
        processor = DrawRectProcessor(
            old_color, new_color, self.selection, self.min_width, self.min_height)
    ...
    def train_models(self, state: ArcTrainingState,
        ...
        all_models = make_all_models(df, self.params, 'draw_rect', labels, label_types)
        return [DrawRect(self.selection, self.feat_index, oldc_model, newc_model,
                         self.params, self.min_width, self.min_height)
