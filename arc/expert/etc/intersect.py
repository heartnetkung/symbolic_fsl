...
class Intersect(ModelFreeArcAction[AttentionTask]):
    def __init__(self, feat_indexes: list[int], color: int)->None:
    ...
    def perform(self, state: ArcState, task: AttentionTask)->Optional[ArcState]:
        ...
        for id1, indexes in zip(atn.sample_index, atn.x_index):
            ...
            for id2 in id2s[1:]:
                new_shape = intersect(new_shape, all_shapes[id1][id2], self.color)
    ...
    def intersect(self, state: ArcState, atn: Attention)->list[Shape]:
        assert state.out_shapes is not None
